...
# Calculate the per-field overall weighed (by sd) averages 
greens    <- c()
nirs      <- c()
reds      <- c()
greens_sd <- c()
...
fields_temp <- fields[fields != "CK_F3" & fields != "CK_F5"] # Remove these fields for this analysis because very high reflectances, causing distorted VI values
...
# Make calculation for green weighing
