...
combis    <- unique(all_palms@data$PR_FI) # CK_F4 missing because of weird naming


combis    <- combis[grepl("F5", combis)]
...
for(COMBI in 1:length(combis)){
  ...
  if(length(inputs) > 0){
    ...
    NDVI_name   <- grep('_NDVI_', ras_names, value=TRUE)
    NDRE_name   <- grep('_NDRE_', ras_names, value=TRUE)
        ...
        for(PALM_ID in 1:nrow(palms_F)){
          ...
          for(BRK_SET in 1:length(DL_brks)){
            ...
            if(brk_D < max_RGB  &  brk_L > min_NIR){
              ...
              zonals_NDRE_1palm_1buf_1brk        <- zonal_stats_tree(PR_FI, NDRE_mask, palm, buf_dist, BRK_SET, palm_name)
              i <- i + 1
              df[i,] <- zonals_NDRE_1palm_1buf_1brk
              
              NDVI_mask                          <- mask(NDVI, mask_ras)
              zonals_NDVI_1palm_1buf_1brk        <- zonal_stats_tree(PR_FI, NDVI_mask, palm, buf_dist, BRK_SET, palm_name)
              i <- i + 1
              df[i,] <- zonals_NDVI_1palm_1buf_1brk
