...
for PROV in provinces:
    ...
    for FIELD in fields:
        ...
        if any(FIELDNAME in s for s in orthos) & any(FIELDNAME in s for s in treats):
            ...
            # Intermediates
            reprojected_shp  = wd + "2_Intermediate\\01_Treats_" + PROV + "_" + FIELDNAME + ".shp"
            output_ortho_tif = wd + "2_Intermediate\\01a3_FieldMask\\01_Ortho_"  + PROV + "_" + FIELDNAME + "_Field_Mask.tif"

##            # Reproject palm points
##            arcpy.Project_management(input_shp, reprojected_shp, crs_in, "", crs_out, "NO_PRESERVE_SHAPE", "", "NO_VERTICAL")

            # Mask orthomosaic
            arcpy.gp.ExtractByMask_sa(input_ortho_tif, input_shp, output_ortho_tif)
